...
def select_veto_muons(df, nMuons=1, condition="==", ptCut=15.0, etaCut=2.4, useGlobalOrTrackerVeto=False):
    ...
    if not useGlobalOrTrackerVeto:
        ...
        df = df.Define("vetoMuonsPre2", "vetoMuonsPre && ((Muon_isGlobal && Muon_standalonePt > 15 && Muon_highPurity && Muon_standaloneNumberOfValidHits > 0 && wrem::vectDeltaR2(Muon_standaloneEta, Muon_standalonePhi, Muon_correctedEta, Muon_correctedPhi) < 0.09) || (Muon_highPurity && Muon_isTracker && Muon_innerTrackOriginalAlgo != 13 && Muon_innerTrackOriginalAlgo != 14))")
