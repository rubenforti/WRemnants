...
def build_graph(df, dataset):
    ...
    if auxiliary_histograms:
        ...
        if isQCDMC:
            ...
            df = df.Define("postfsrMuonsStatus1_pt0", "GenPart_pt[postfsrMuonsStatus1genMatched][0]")
            df = df.Define("postfsrMuonsStatus1_isPrompt0", "GenPart_isPrompt[postfsrMuonsStatus1genMatched][0]")
            leadPtGenMatchStatus1 = df.HistoBoost("leadPtGenMatchStatus1", [axis_genPt, axis_prompt, axis_genPartFlav], ["postfsrMuonsStatus1_pt0", "postfsrMuonsStatus1_isPrompt0", "goodMuons_genPartFlav0", "nominal_weight"])
            results.append(leadPtGenMatchStatus1)
            genPtvsRecoPt = df.HistoBoost("genPtvsRecoPt", [axis_genPt, axis_pt, axis_genPartFlav, axis_passIso], ["postfsrMuonsStatus1_pt0", "goodMuons_pt0", "goodMuons_genPartFlav0", "passIso", "nominal_weight"])
            results.append(genPtvsRecoPt)
            #
