...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-x",
        "--excludeNuisgroups",
        default=".*refire|.*CMS_back|.*Luminosity|.*lumi|.*expNoCalib|.*Fake.+|.*eff_(stat|syst).+|.*eff_all|.*AlphaS$|.*nonClosure|.*resolutionCrctn|.*scaleCrctn|.*scaleClos|.*polVar|.*QCDscale$|.*QCDscale(W|Z)|.*resum|.*(muon|ecal)Prefire|FakeRate|theory_ew_|.*pixel|theory$|experiment$|bcQuark|helicity_shower|.*widthW|.*ZmassAndWidth|.*sin2thetaZ",
        type=str,
