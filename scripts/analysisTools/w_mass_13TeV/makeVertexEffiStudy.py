...
def getVertexEfficiency(h, rebin=1, dzCut_cm=0.1, label=""):
    ...
    hptVtx1mm = h[{"absDiffGenRecoVtx_z" : s[0:complex(0,dzCut_cm + 0.0001):hist.sum]}]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--baseName", type=str, help="Histogram name in the file", default="nominal_vertexZstudy")
    ...
    parser.add_argument('-p','--process', default="Wmunu", choices=["Wmunu", "Zmumu"], type=str, help='Choose what process to plot')
    parser.add_argument(     '--Zdilepton', action='store_true',   help='When using Z process, do stuff for a dilepton selection with both muons in acceptance')
    ...
    for d in datasets:
        ...
        for ieta,eta in enumerate(absEtaEdges4Plot):
            ...
            etaBinLabel = "abseta" + str(eta).replace(".","p") + "to" + str(etaUp).replace(".","p")
            etaBinLegEntry.append("%s < |^{ }#eta^{#mu }| < %s" % (round(eta,1), round(etaUp,1)))
            if eta == 0.0:
                etaBinLegEntry[-1] = "|^{ }#eta^{#mu }| < %s" % round(etaUp,1)
            # backward compatibility
            if "abseta" in hin.axes.name: 
                h = hin[{"abseta" : s[complex(0,eta):complex(0,nAbsEtaBins):hist.sum],
                         "passIso": True,
                         "passMT" : True}]
            else:
                h = hin[{"abeta" : s[complex(0,eta):complex(0,nAbsEtaBins):hist.sum],
                         "passIso": True,
                         "passMT" : True}]                
    ...
    colors = [ROOT.TColor.GetColor("#e42536"), ROOT.TColor.GetColor("#5790fc"), ROOT.TColor.GetColor("#964A8B"), ROOT.TColor.GetColor("#f89c20"), ROOT.TColor.GetColor("#9c9ca1")]
    markers = []
    ...
    drawGraphCMS([gr_vpts[wp] for wp in wps], xAxisName, f"{yAxisName}::{ymin},1.0", f"vtxEff_genBosonPt_{postfixForCanvasName}", outdir,
                 leg_roc=etaBinLegEntry[:], legendCoords = "0.52,0.18,0.92,0.39;1",
                 passCanvas=canvas, etabinText=textForPlot, skipLumi=False, drawLumiLatex=True,
                 useOriginalGraphStyle=True, graphDrawStyle="EP", legEntryStyle="EP", cmsText=args.CMStext,
                 solidLegend=True)
