...
def plot_closure(h, outdir, suffix="", outfile=f"closureABCD", ratio=True, proc="", ylabel="a.u.", smoothing_mode="binned", normalized=False, bootstrap=False):
    ...
    if smoothing_mode in ["full", "fakerate"]:
        ...
        hSel_ext8 = sel.FakeSelector2DExtendedABCD(h, **info, upper_bound_y=None,
            integrate_shapecorrection_x=True, interpolate_x=False, smooth_shapecorrection=False)
    ...
    if ratio:
        ...
        if smoothing_mode in ["binned"]:
            ...
            labels = [fr"{l} $\chi^2/ndf={round(c)}/{ndf}$" if i!= 0 else l for i, (l, c) in enumerate(zip(labels, chi2s))]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--procFilters", type=str, nargs="*", default=["QCD"], help="Filter to plot (default no filter, only specify if you want a subset")
